...
export default function RandomQuestionPanel({ teamId, onCorrect }) {
  ...
  const recordResponse = useCallback(async (correct, preguntaId, materiaId) => {
    ...
    try {
      await fetch('https://v62mxrdy3g.execute-api.us-east-1.amazonaws.com/prod/recordResponseRDS', {
        method: 'POST',
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      const resp = await fetch('https://v62mxrdy3g.execute-api.us-east-1.amazonaws.com/prod/obtenerPreguntasRDS', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!Array.isArray(data) || data.length === 0) throw new Error('No hay preguntas disponibles')
  ...
  return (
    <div className="question-panel">
      <div className="question-container">
        <h3 className="question-text">{current.enunciado}</h3>
          ...
          {current.opciones.map((op, idx) => (
            <button
              ...
              className={`option-btn${
                ...
                showFeedback && selectedOption === op && !op.correcta ? ' incorrect' : ''
              }`}
      ...
      {showFeedback && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>{selectedOption.correcta ? '¡Correcto!' : 'Incorrecto'}</h3>
            <p className="justificacion">{current.justificacion}</p>
            <button className="next-button" onClick={handleNext}>Siguiente Pregunta</button>
          </div>
        </div>
      )}
    </div>
